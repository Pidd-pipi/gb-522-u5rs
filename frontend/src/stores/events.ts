...
import type { EventMarker, EventType, ReviewEventResponse } from '@/types/event'
...
export const useEventStore = defineStore('events', () => {
  ...
  async function review(id: number, body: { event_type: EventType; distance_m?: number; review_note: string; version: number }): Promise<ReviewEventResponse> {
    const { data } = await eventApi.review(id, body)
    const index = items.value.findIndex((item) => item.id === id)
    if (index >= 0) items.value[index] = data.data.event
    return data.data
  }
