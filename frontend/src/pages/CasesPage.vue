  ...
  <section class="content-band">
    ...
    <div class="data-surface">
      <el-table v-loading="cases.loading" :data="cases.items" row-key="id">
        ...
        <el-table-column label="状态" width="150"><template #default="scope"><span class="status-cell"><span class="status-pill" :class="scope.row.case_status">{{ caseStatusLabel[scope.row.case_status as LocalizationCase['case_status']] }}</span><el-tooltip v-if="scope.row.invalidation_reason" :content="scope.row.invalidation_reason" placement="top"><span class="stale-tag"><TriangleAlert :size="12" />需重新分析</span></el-tooltip></span></template></el-table-column>
...
<style scoped>
.case-toolbar{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-bottom:14px}.state-track{display:flex;align-items:center;overflow:auto}.state-track span{display:flex;align-items:center;gap:6px;color:var(--text-muted);font-size:11px;font-weight:700;white-space:nowrap}.state-track span:not(:last-child)::after{content:'';width:24px;height:1px;margin:0 7px;background:var(--line-strong)}.state-track i{width:20px;height:20px;display:grid;place-items:center;border:1px solid var(--line-strong);border-radius:50%;font-style:normal}.trace-pair{display:inline-flex;align-items:center;gap:6px}.uncertainty{display:block;margin-top:2px;color:var(--text-muted)}.conclusion{display:-webkit-box;overflow:hidden;-webkit-line-clamp:2;-webkit-box-orient:vertical}.muted{color:var(--text-muted);font-size:12px}.stale-tag{display:inline-flex;align-items:center;gap:4px;margin-top:6px;padding:2px 8px;border:1px solid var(--warning);border-radius:999px;color:var(--warning);font-size:11px;font-weight:700;cursor:help}.status-cell{display:flex;flex-direction:column;align-items:flex-start}.case-form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 14px}@media(max-width:700px){.case-toolbar{align-items:stretch;flex-direction:column}.case-form-grid{grid-template-columns:1fr}}
</style>
